...
class temporalBAStack():
    ...
    def stackSpatialExtent(self, bounding_extents_file):
        ...
        if not os.path.exists (bounding_extents_file):
            logger.error('Bounding extents file does not exist: {0}'
                         .format(bounding_extents_file))
            return None
    ...
    def sceneResample(self, xml_file):
        ...
        if xmlAttr is None:
            logger.error('Error reading the XML and setting up the bands: {0}'
                         .format(xml_file))
            return ERROR
        ...
        if specIndx == None:
            logger.error('Error generating the spectral indices for {0}'
                         .format(xml_file))
            return ERROR
        ...
        if status != SUCCESS:
            logger.error('Error creating the spectral indices for {0}'
                         .format(xml_file))
            return ERROR
    ...
    def generateYearSeasonalSummaries (self, year, season):
        ...
        for ind in ['band3', 'band4', 'band5', 'band7', 'ndvi', \
            ...
            for i in range(0, n_files):
                ...
                if my_ds is None:
                    logger.error('Could not open index/band file: {0}'
                                 .format(temp_file))
                    return ERROR
                input_ds[i] = my_ds
...
#                    logger.debug('  Stacking file: ' + files[i])
    ...
    def processStack (self, input_dir=None, exclude_l1g=None,
        ...
        logger.info('Burned area temporal stack processing of directory: {0}'
                    .format(input_dir))
